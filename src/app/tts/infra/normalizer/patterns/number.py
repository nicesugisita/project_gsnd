...
class NumberHandler(PatternHandler):
    """
    숫자 패턴 핸들러 (금액 단위 '만/억/조' 및 천단위 콤마 완벽 대응 버전)
    """
    ...
    def _load_hardcoded_config(self) -> None:
        ...
        self.sino_units = ['번', '회', '분', '초', '일', '주', '년', '월', '호', '층', '차', '건', '개월']
    ...
    def _compile_patterns(self) -> None:
        ...
        # [핵심 수정 2] 금액 패턴 강화: '원' 뿐만 아니라 '만원', '억원', '조원' 등 큰 단위 금액까지 한 번에 포착
        self.currency_pattern = re.compile(r'([+-]?\d{1,3}(?:,\d{3})+|[+-]?\d+)\s*((?:만|억|조|경)?\s*원)')
        ...
        # 5. 일반 숫자 (콤마가 포함된 숫자가 일반 숫자로 잡혀 쪼개지는 것을 방지하기 위해 콤마 패턴 포함)
        self.number_only_pattern = re.compile(r'(?<!\d)([+-]?\d{1,3}(?:,\d{3})+|[+-]?\d+)(?!\d)')
    ...
    def _currency_to_korean(self, match: re.Match) -> str:
        # [핵심 수정] 콤마(,)를 확실하게 제거하여 '3,500'을 '3500' 정수로 만듭니다.
        num_str = match.group(1).replace(',', '')
        unit = match.group(2).strip() # 공백 제거 (예: '만 원' -> '만원')
        prefix = "마이너스 " if num_str.startswith('-') else ""
    ...
    def _convert_to_full_korean(self, num_str: str) -> str:
        try:
            # 일반 숫자 처리 시에도 콤마를 지우고 순수 숫자로 계산하게 유도합니다.
            clean_num = num_str.replace(',', '')
            prefix = "마이너스 " if clean_num.startswith('-') else ""
            return prefix + self._convert_number_to_sino(int(clean_num.lstrip('+-')))
        except:
            return num_str

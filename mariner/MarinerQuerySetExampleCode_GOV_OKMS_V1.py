...
def __get_jar_files():
    lib_path = os.path.normpath(os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', '..', 'jar_lib'))
...
# not
# whereset id_색인필드 hasany "A B C D E F G"

intent = "more_info"
#exclude_chunk_id = ["WLF00000022" ,"WLF00001185"]
exclude_chunk_id = []

if intent == "more_info" and exclude_chunk_id:
	for chunk_id in exclude_chunk_id:
		where_set_array += [
			jpkg.WhereSet(7),
			jpkg.WhereSet("SERVICE_ID", 33, chunk_id, 0),
		]
...
for i in range(len(doc_service_id_list)):
	print(
		f"서비스 ID = {doc_service_id_list[i]}\n",
		# f"서비스명={doc_service_name_list[i]}\n",

...
collection_name = 'GSND_DATASET_V8'
threshold = 0.2
top_n = 20
question = "창원 노인맞춤돌봄서비스"
...
doc_content_field         = jpkg.SelectSet(JString("CHUNK_PATH"),            num, 0)
...
doc_compli_dt_field      = jpkg.SelectSet(JString("COMPLI_DT"),         num, 0)
doc_label_field          = jpkg.SelectSet(JString("LABEL"),            num, 0)

select_set_array = [
    doc_chunk_id_field,           # index 0
    doc_name_field,               # index 1
    doc_content_field,            # index 2
    doc_sigun_field,              # index 3
    weight_field,                 # index 4
    doc_compli_dt_field,          # index 5
    doc_label_field,              # index 6
]
...
compli_dt = "2026"
date = "2025"
# # WhereSet 구성 (벡터 인덱스 필드명은 Mariner 컬렉션 스키마에서 확인 필요)
# where_set_array = [
# 	jpkg.WhereSet(9),
# 	# 색인필드, 프로토콜, 키워드, 가중치(비율)
# 	jpkg.WhereSet("NAME_KO", 2, keyword_string, 0.7),
# 	jpkg.WhereSet(6),  # Protocol.WhereSet.OP_OR 0x06
# 	jpkg.WhereSet("TEXT_CHUNK_KO", 2, keyword_string, 0.7),
# 	jpkg.WhereSet(6),  # Protocol.WhereSet.OP_OR 0x06
# 	jpkg.WhereSet("NAME_MI", 2, keyword_string, 0.7),
# 	jpkg.WhereSet(6),
...
if date == "" :  #검색어에서 날짜 정보가 없을때에는 현재 날짜 구해와서 가중치 점수 추가
  	where_set_array += [
        jpkg.WhereSet(8),
        # jpkg.WhereSet("COMPLI_DT", 1, "현재날짜년도", 1.0)
        jpkg.WhereSet("COMPLI_DT", 1, "2026", 1.0)
    ]
else :  # 검색어에 날짜 정보가 있는데 범위가 아닌 매칭 검색인경우 
	where_set_array += [
        jpkg.WhereSet(8),
        jpkg.WhereSet("COMPLI_DT", 1, date, 1.0)
    ]
...
order_set_array.append(jpkg.OrderBySet(True, "COMPLI_DT", jpype.JByte(97))) # 문서 날짜 순 정렬: 97 = OP.PRE_WEIGHT >>> 가중치 우선 정렬 후 같을 경우 COMPLI_DT(정렬필드)로 정렬
...
# 검색어에 날짜 정보가 있는데 범위 검색인 경우 
filter_set_array = [jpkg.FilterSet(jpype.JByte(3), "COMPLI_DT", jpype.JArray(jpype.JString)(["20260101", "20261231"]), 0)]
...
if resultSet is None:
	...
	if not result_size == 0:
		for i in range(result_size):
			...
			doc_content_list.append(str(result.getResult(i, 1)))
			doc_sigun_list.append(str(result.getResult(i, 2)))
			doc_name_list.append(str(result.getResult(i, 3)))
			print(i, result.getResult(i, 4))
			doc_score = float(str(result.getResult(i, 4))) * 0.0001
			doc_compli_dt_list.append(str(result.getResult(i, 5)))
			doc_score_list.append(str(doc_score))
			doc_label_list.append(str(result.getResult(i, 6)))
...
for i in range(len(doc_chunk_id_list)):
	print(f"[{i}] 청크ID={doc_chunk_id_list[i]},\n사업명={doc_name_list[i]},\n"
		f"공고일={doc_compli_dt_list[i]}\n유사도={doc_score_list[i]}\n",
		f"내용={doc_content_list[i]}\n",
		f"라벨={doc_label_list[i]},\n",
		f"==========================================="
	)
	logger.debug(
		f"[{i}] 청크ID={doc_chunk_id_list[i]} "
		f"라벨={doc_label_list[i]}, 유사도={doc_score_list[i]}"
	)

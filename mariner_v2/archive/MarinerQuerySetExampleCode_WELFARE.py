...
collection_name = 'GSND_WELFARE_CENTER_V1'
threshold = 0.2
top_n = 20
question = "창원 노인복지센터 알려줘"

doc_id_list = []
doc_sigun_list = []
doc_facility_name_list = []
doc_address_list = []
doc_facility_type_list = []
doc_homepage_list = []
doc_tel_list = []
doc_score_list = []
...
doc_facility_name_field   = jpkg.SelectSet(JString("FACILITY_NAME"),               num, 0)
doc_address_field         = jpkg.SelectSet(JString("ADDRESS"),            num, 0)
doc_facility_type_field  = jpkg.SelectSet(JString("FACILITY_TYPE"),              num, 0)
doc_homepage_field       = jpkg.SelectSet(JString("HOMEPAGE"),              num, 0)
doc_tel_field            = jpkg.SelectSet(JString("TEL"),              num, 0)
weight_field              = jpkg.SelectSet(JString("WEIGHT"),             num, 0)

select_set_array = [
    doc_id_field,           # index 0
    doc_facility_name_field,               # index 1
    doc_address_field,            # index 2
    doc_sigun_field,              # index 3
    doc_facility_type_field,                 # index 4
    doc_homepage_field,          # index 5
    doc_tel_field,               # index 6
    weight_field,                # index 7
]
...
where_set_array = [
    ...
    jpkg.WhereSet(6),
    jpkg.WhereSet("FACILITY_NAME", 2, keyword_string, 0.7),
    jpkg.WhereSet(6),
...
if resultSet is None:
	...
	if not result_size == 0:
		for i in range(result_size):
			...
			doc_facility_name_list.append(str(result.getResult(i, 1)))
			doc_address_list.append(str(result.getResult(i, 2)))
			doc_sigun_list.append(str(result.getResult(i, 3)))
			doc_facility_type_list.append(str(result.getResult(i, 4)))
			doc_homepage_list.append(str(result.getResult(i, 5)))
			doc_tel_list.append(str(result.getResult(i, 6)))
			print(i, result.getResult(i, 7))
			doc_score = float(str(result.getResult(i, 7))) * 0.0001
			doc_score_list.append(str(doc_score))

logger.debug(f"Mariner 검색 결과 수: {len(doc_id_list)}")
for i in range(len(doc_id_list)):
	print(f"[{i}] 청크ID={doc_id_list[i]},\n시설명={doc_facility_name_list[i]},\n"
		f"유사도={doc_score_list[i]}\n",
		# f"시군={doc_sigun_list[i]},\n",
		f"==========================================="
	)
	logger.debug(
		f"[{i}] 청크ID={doc_id_list[i]} "

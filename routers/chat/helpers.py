...
def _get_rag_processor(intent: str):
    """intent에 따라 적절한 RAG 처리 함수를 반환합니다."""
...
async def _handle_rag_mode(
    ...
    from utils.status_messages import build_status_message, STATUS_QUERY_REFORM
    from services.tts_service import request_tts_stream

    # 시군 체크: sigun_filters가 이미 제공된 경우 건너뜀
    ...
    if stream:
        ...
        async def rag_stream():
            ...
            rag_processor = _get_rag_processor(intent)
        ...
        conv_id = await asyncio.to_thread(_save_chat_history, chat_request, response_message, user_message)
